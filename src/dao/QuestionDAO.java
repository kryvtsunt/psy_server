...
public class QuestionDAO {
	...
	public static JSONObject getAllQuestionsByTargetGroupId(Long targetGroupId){
		...
		String selectQuery = "SELECT D.NAME AS questionName, F.FIELDNAME AS responseType, "
				+ "D.ID AS questionId, E.STARTLABEL AS startLabel, E.ENDLABEL AS endLabel, E.ADDITIONALLABEL AS additionalLabel, E.FOLLOWUP AS followup "
				+ "FROM TARGETGROUP AS A INNER JOIN TRAINING AS B ON A.TRAININGID = B.ID "
				+ "INNER JOIN TRAININGQUESTIONMAP AS C ON B.ID = C.TRAININGID "
			...
			while (rows.next()){
				...
				object.put(Constant.QUESTION_CATEGORY_ADDITIONAL_LABEL, rows.getString("additionalLabel"));
				object.put(Constant.QUESTION_CATEGORY_FOLLOWUP, rows.getInt("followup"));
