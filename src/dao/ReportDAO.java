...
public class ReportDAO {
	...
	public static JSONObject getAvgResponseTimeForImageResponsesHelper(Long pId, int correctness) throws SQLException{
		...
		String selectQuery = "select a.sessionId, IFNULL(d.average, 0) as average, "
				...
				+ "from IMAGERESPONSE as a "
				+ "left join IMAGE as b on a.imageId = b.id "
				+ "left join IMAGECATEGORY as c on c.id=b.categoryId "
				+ "left join FIELDLOOKUP as e on e.id=b.imageType "
				+ "left join "
				+ "(select c.sessionId, c.participantId, AVG(c.timeTaken) as average, e.name, f.fieldName "
				+ "from IMAGERESPONSE as c "
				+ "inner join IMAGE as b on c.imageId = b.id "
				+ "inner join IMAGECATEGORY as e on e.id=b.categoryId "
				+ "inner join FIELDLOOKUP as f on f.id=b.imageType "
	...
	public static String generateImageReportFileForTargetGroup(Long tgId){
		...
		String selectQuery = "select a.name as targetGroup, b.username as user, "
				...
				+ "from TARGETGROUP as a "
				+ "inner join PARTICIPANT as b on a.id = b.targetGroupId "
				+ "inner join IMAGERESPONSE as c on c.participantId = b.id "
				+ "inner join IMAGE as d on d.id = c.imageId "
				+ "inner join IMAGECATEGORY as e on e.id = d.categoryId "
				+ "inner join FIELDLOOKUP as f on f.id = d.imageType "
				+ "inner join USERSESSION as g on g.id = c.sessionId "
	...
	public static String generateQuestionReportFileForTargetGroup(Long tgId){
		...
		String selectQuery = "select a.name as targetGroup, b.username as user, c.sessionId as sessionId, "
				...
				+ "from TARGETGROUP as a "
				+ "inner join PARTICIPANT as b on a.id = b.targetGroupId "
				+ "inner join QUESTIONRESPONSE as c on c.participantId = b.id "
				+ "inner join QUESTION as d on d.id = c.questionId "
				+ "inner join QUESTIONCATEGORY as e on e.id = d.categoryId "
				+ "inner join FIELDLOOKUP as f on f.id = e.responseType "
				+ "inner join USERSESSION as g on g.id = c.sessionId "
